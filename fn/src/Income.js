...
export default function FormDialog() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (Edit) {
      axios.post(`http://localhost:5000/User/updateIncome?id=${Edit}`, { arr: { ...data } }, {
        headers: {
      ...
      axios.post(`http://localhost:5000/User/Income`, { arr: { ...data, Date: new Date().toLocaleString() } }, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      }).then(() => {
  ...
  const exportToPDF = () => {
    const docDefinition = {
      content: [
        { text: 'Income Data', style: 'header' },
        {
          table: {
            headerRows: 1,
            widths: ['auto', '*', 'auto', 'auto', '*'],
            body: [
              ['Index', 'Date', 'Income', 'Category', 'Description'],
    ...
    try {
      pdfMake.createPdf(docDefinition).download('Income_Data.pdf');
    } catch (error) {
  ...
  const handleSearch = (e) => {
    ...
    if (term.trim() === '') {
      ...
      const filtered = arr.filter((row) =>
        ...
        row.Income?.toString().includes(term) ||
  ...
  // Sorting functions
  const sortByIncomeAsc = () => {
    const sorted = [...filteredArr].sort((a, b) => (Number(a.Income) || 0) - (Number(b.Income) || 0));
    setFilteredArr(sorted);
    handleSortClose();
  };

  const sortByIncomeDesc = () => {
    const sorted = [...filteredArr].sort((a, b) => (Number(b.Income) || 0) - (Number(a.Income) || 0));
    setFilteredArr(sorted);
    handleSortClose();
  };
  ...
  React.useEffect(() => {
    axios.get("http://localhost:5000/User/getIncome", {
      headers: {
        Authorization: `Bearer ${localStorage.getItem('token')}`,
      },
    }).then((response) => {
      const incomeData = response.data.data.Income;
      setArr(incomeData);
      setFilteredArr(incomeData);
    }).catch((err) => {
      console.log(err);
    });
  }, [update, filterType]);
  ...
  return (
    <Sidebar navigatePathName="/dashboard/income">
      <Navbar />
      ...
      <Dialog open={open} onClose={handleClose}>
        <form onSubmit={handleSubmit}>
          <DialogTitle>{Edit ? "Edit" : "Add"} Income Details</DialogTitle>
          <DialogContent>
            <DialogContentText>
              <TextField
                autoFocus
                ...
                id="Date"
                name="Date"
                value={Edit ? data.Date : new Date().toLocaleString()}
                disabled
                type="text"
                fullWidth
                variant="standard"
        ...
        <Grid item xs={12}>
          <Grid container justifyContent="end" sx={{ mb: 2 }} spacing={2}>
            ...
            <Grid item xs={6} sm={2} md={2}>
              ...
              >
                <MenuItem onClick={sortByIncomeAsc}>Sort by Income (Asc)</MenuItem>
                <MenuItem onClick={sortByIncomeDesc}>Sort by Income (Desc)</MenuItem>
                    ...
                    <TableRow key={row.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                      ...
                      <TableCell align="center">
                        <Button
                          ...
                          style={{ color: "white", backgroundColor: "black" }}
                          onClick={() => {
                            axios.delete(`http://localhost:5000/User/deleteIncome/?id=${row.id}`, {
                              headers: {
                                Authorization: `Bearer ${localStorage.getItem('token')}`,
                              },
                            }).then(() => {
